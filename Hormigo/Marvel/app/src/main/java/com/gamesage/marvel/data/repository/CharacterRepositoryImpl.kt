...
class CharacterRepositoryImpl @Inject constructor(
    ...
    override suspend fun update(character: Character): Result<Int> {
        return localDataSource.update(character)
    }

    override fun observe(): Flow<Result<List<Character>>> {
        scope.launch {
            val localResult = localDataSource.readAll()
            if (localResult.isSuccess && localResult.getOrNull().isNullOrEmpty()) {
                val remoteResult = remoteDataSource.readAll()
                if (remoteResult.isSuccess) {
                    remoteResult.getOrNull()?.let { localDataSource.addAll(it) }
                }
            }
        }
