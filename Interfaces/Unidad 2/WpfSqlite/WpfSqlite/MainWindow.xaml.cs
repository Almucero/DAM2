    ...
    public partial class MainWindow : Window
    {
        ...
        private void CrearTablas()
        {
            try
            {
                ...
                string tablaZoo = @"CREATE TABLE IF NOT EXISTS Zoo(
                                    Id INTEGER PRIMARY KEY AUTOINCREMENT,
                                    Ubicacion TEXT NOT NULL);";

                string tablaAnimal = @"CREATE TABLE IF NOT EXISTS Animal(
                                       Id INTEGER PRIMARY KEY AUTOINCREMENT,
                                       Nombre TEXT NOT NULL);";

                string tablaRelacion = @"CREATE TABLE IF NOT EXISTS AnimalZoo(
                                         Id INTEGER PRIMARY KEY AUTOINCREMENT,
                                         ZooId INTEGER,
                                         AnimalId INTEGER,
        ...
        private void MuestraAnimalesAsociados()
        {
            try
            {
                ...
                string consulta = @"SELECT a.Nombre, a.Id 
                                    FROM Animal a 
        ...
        private void MuestraZooElegidoEnTextBox()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                    miTextBox.Text = dt.Rows[0]["Ubicacion"].ToString();
            }
            catch (Exception ex)
            {
        ...
        private void MuestraAnimalElegidoEnTextBox()
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                    miTextBox.Text = dt.Rows[0]["Nombre"].ToString();
            }
            catch (Exception ex)
            {
        ...
        private void AgregarZoo_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand(
                    "INSERT INTO Zoo (Ubicacion) VALUES (@Ubicacion)", connection);
        ...
        private void EliminarZoo_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand(
                    "DELETE FROM Zoo WHERE Id=@Id", connection);
                cmd.Parameters.AddWithValue("@Id", ListaZoos.SelectedValue);
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
        ...
        private void AgregarAnimalZoo_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand(
                    "INSERT INTO AnimalZoo (ZooId, AnimalId) VALUES (@Z, @A)", connection);
        ...
        private void ActualizarZoo_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                connection.Open();
                SQLiteCommand cmd = new SQLiteCommand(
                    "UPDATE Zoo SET Ubicacion=@u WHERE Id=@id", connection);
        ...
        private void AgregarAnimal_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand(
                    "INSERT INTO Animal (Nombre) VALUES (@n)", connection);
        ...
        private void EliminarAnimal_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand(
                    "DELETE FROM Animal WHERE Id=@id", connection);
                cmd.Parameters.AddWithValue("@id", ListaAnimales.SelectedValue);
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
        ...
        private void ActualizarAnimal_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                connection.Open();
                SQLiteCommand cmd = new SQLiteCommand(
                    "UPDATE Animal SET Nombre=@n WHERE Id=@id", connection);
                cmd.Parameters.AddWithValue("@n", miTextBox.Text);
                cmd.Parameters.AddWithValue("@id", ListaAnimales.SelectedValue);
                cmd.ExecuteNonQuery();
            }
        ...
        private void QuitarAnimalDelZoo_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SQLiteCommand cmd = new SQLiteCommand(
                    "DELETE FROM AnimalZoo WHERE ZooId=@z AND AnimalId=@a", connection);
                cmd.Parameters.AddWithValue("@z", ListaZoos.SelectedValue);
